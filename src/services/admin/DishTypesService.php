...
class DishTypesService extends MvcService
{
  ...
  public function get_all_default_dish_types(): array
  {
    ...
    return array(
      ...
      'redirect_admin_link' => array('redirect_link' => 'admin'),
    );
  }
  ...
  public function add_dish_type()
  {
    try {
      ...
      if ($statement->fetchColumn() != 0) {
        throw new Exception('Podana nazwa potrawy istnieje już w systemie. Spróbuj wprowadzić inną nazwę potrawy i zatwierdź zmiany.');
      }
      $query = "INSERT INTO dish_types (name) VALUES (LOWER(?))";
      $statement = $this->dbh->prepare($query);
      $statement->execute(array($v_name['value']));

      $this->_banner_message = 'Nowy typ potrawy o nazwie <strong>' . $v_name['value'] . '</strong> został pomyślnie dodany do systemu.';
      $statement->closeCursor();
      if ($this->dbh->inTransaction()) {
        $this->dbh->commit();
      }
