...
use App\Models\User\EditUserPersonalDataModel;
...
class SettingsService extends MvcService
{
  ...
  public function get_and_modify_user_personal_data()
  {
    ...
    try {
      ...
      $query = "
        ...
        FROM users AS u
        INNER JOIN user_addresses AS a ON a.user_id = u.id
        WHERE u.id = ?
      ";
      ...
      if (isset($_POST['user-edit-data'])) {
        ...
        if (!empty($_POST['user-locale-no'])) {
          ...
          $user->locale_no = array('value' => $_POST['user-locale-no'], 'invalid' => false, 'bts_class' => '');
        }
        ...
        if ($user->all_is_valid()) {
          ...
          if ($statement->fetchColumn() > 0) {
            throw new Exception('Użytkownik z podanymi loginem lub adresem email istnieje już w systemie. Spróbuj wpisać inny login.');
          }
          ...
          SessionHelper::create_session_banner(SessionHelper::ADMIN_PROFILE_PAGE_BANNER, $this->_banner_message,
            $this->_banner_error);
          header('Refresh:0; url=' . __URL_INIT_DIR__ . 'admin/profile', true, 301);
          die;
      ...
      $this->dbh->rollback();
      SessionHelper::create_session_banner(SessionHelper::ADMIN_SETTINGS_PAGE_BANNER, $e->getMessage(), true);
    }
    return array(
      'user' => $user,
  ...
  public function delete_profile_image()
  {
    ...
    SessionHelper::create_session_banner(SessionHelper::ADMIN_SETTINGS_PAGE_BANNER, $this->_banner_message, $this->_banner_error);
  }
}
