...
use App\Models\User\AdminUserDetailsModel;
...
class ManageUsersService extends MvcService
{
  ...
  /**
   * Metoda odpowiadająca za usunięcie wybranego przez administratora użytkownika.
   */
  public function delete_user()
  {
    ...
    try {
      ...
      if (!$result) {
        throw new Exception('Podany użytkownik nie istnieje w systemie lub została wcześniej usunięty.');
      }
  ...
  public function get_users(): array
  {
    ...
    try {
      ...
      $statement->execute();
      while ($row = $statement->fetchObject(UserDetailsModel::class)) {
        $users_list[] = $row;
      }
  ...
  public function get_users_details()
  {
    ...
    try {
      ...
      $query = "
        ...
        FROM ((users AS u
        INNER JOIN user_addresses AS a ON a.user_id = u.id AND a.is_prime = 1)
        INNER JOIN roles AS r ON u.role_id = r.id)
        WHERE u.id = ?
      ";
      $statement = $this->dbh->prepare($query);
   ...
   */
  public function delete_user_profile_image(): ?string
  {
    if (!isset($_GET['id'])) {
      return null;
    }
    $redirect_path = 'admin/manage-users';
    ...
    try {
      ...
      if ($statement->fetchColumn() == 0) {
        throw new Exception('Wybrany użytkownik nie istnieje w systemie.');
      }
      $redirect_path .= '/user-details?id=' . $_GET['id'];
      ...
      if (empty($data['photo_url'])) {
        throw new Exception('Wybrany użytkownik nie posiada żadnego zdjęcia profilowego.');
      }
