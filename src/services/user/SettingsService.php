...
use App\Models\User\AddNewAddressUserModel;
use App\Models\User\EditUserProfileModel;
...
ResourceLoader::load_model('AddNewAddressUserModel', 'User');
...
class SettingsService extends MvcService
{
   ...
   */
  public function edit_user_profile(): array
  {
    $addresses = array();
    ...
    try {
      ...
      $query = "
        ...
        FROM users u INNER JOIN user_addresses a ON u.id = a.user_id
        WHERE users.id = ?
      ";
      ...
      if (isset($_POST['save-changes-button'])) {
        ...
        if (!empty($_POST['local-number'])) {
          ...
          $user->locale_nr = array('value' => $_POST['local-number'], 'invalid' => false, 'bts_class' => '');
        }
        ...
        if ($user->all_is_valid()) {
          ...
          if ($statement->fetchColumn() > 0) {
            throw new Exception('Podany numer telefonu jest już zarejestrowany na innym koncie.');
          }
          $profile_url = ImagesHelper::upload_user_profile_image(
            $user->profile_url, $_SESSION['logged_user']['user_id'], $profile_photo,
          );
          // Sekcja zapytań dodająca wprowadzone dane do tabel users i user_addresses
          $query = "
          ...
          $query = "
            UPDATE user_addresses SET street = ?, building_nr = ?, locale_nr = NULLIF(?,''), post_code = ?, city = ?
      ...
      $query = "
        ...
        FROM user_addresses WHERE user_id = ? AND is_prime = 0
      ";
      $statement = $this->dbh->prepare($query);
      $statement->execute(array($_SESSION['logged_user']['user_id']));
    ...
    return array(
      ...
      'hide_profile_preview_class' => $user->profile_url['invalid'] ? 'display-none' : '',
  ...
  {
    $redirect_banner = SessionHelper::USER_SETTINGS_PAGE_BANNER;
    $redirect = 'user/settings';
    if (isset($_GET['redirect'])) {
      $redirect_banner = SessionHelper::ORDER_SUMMARY_PAGE_BANNER;
      $redirect = $_GET['redirect'];
    }
    $user = new AddNewAddressUserModel;
    try {
      $this->dbh->beginTransaction();

      $query = "SELECT count(*) FROM user_addresses WHERE user_id = ?";
      $statement = $this->dbh->prepare($query);
      $statement->execute(array($_SESSION['logged_user']['user_id']));
      if ($statement->fetchColumn() == 4) {
        ...
        SessionHelper::create_session_banner($redirect_banner, $message, true);
        header('Location:' . __URL_INIT_DIR__ . $redirect, true, 301);
        die;
      }
      if (isset($_POST['save-changes-add'])) {
        ...
        if ($user->all_is_valid()) {
          // Sekcja zapytań dodająca wprowadzone dane do tabel users i user_addresses
          $query = "
            INSERT INTO user_addresses (street, building_nr, locale_nr, post_code, city, user_id)
            VALUES (?,?,NULLIF(?,''),?,?,?)
          ";
          $statement = $this->dbh->prepare($query);
          $statement->execute(array(
          ...
          SessionHelper::create_session_banner($redirect_banner, $this->_banner_message, $this->_banner_error);
          header('Location:' . __URL_INIT_DIR__ . $redirect, true, 301);
          die;
        }
      }
    ...
    return array(
      'user' => $user,
      'redirect_active' => isset($_GET['redirect']),
  ...
  public function delete_address()
  {
    ...
    try {
      $this->dbh->beginTransaction();
      $query = "SELECT count(*) FROM user_addresses WHERE user_id = ? AND is_prime = 0";
      $statement = $this->dbh->prepare($query);
      $statement->execute(array($_SESSION['logged_user']['user_id']));

      if ($statement->fetchColumn() == 0) {
        throw new Exception('Użytkownik nie posiada alternatywnych adresów');
      }
      $query = " DELETE FROM user_addresses WHERE user_id = ? AND is_prime = 0 AND id = ?";
      $statement = $this->dbh->prepare($query);
   ...
   */
  public function edit_alternative_address(): array
  {
    if (!isset($_GET['id'])) {
      header('Location:' . __URL_INIT_DIR__ . 'user/settings', true, 301);
    }
    ...
    try {
      ...
      if (isset($_POST['save-changes-add'])) {
        ...
        if ($user->all_is_valid()) {
          $query = "
            UPDATE user_addresses SET street = ?, building_nr = ?, locale_nr = ?, post_code = ?, city = ?
  ...
  public function delete_profile_image()
  {
    try {
      ...
      if ($photo_url === 0) {
        throw new Exception('Wybrany użytkownik nie posiada zdjęcia profilowego.');
      }
  ...
  public function delete_account()
  {
    ...
    try {
      ...
      if (!$user_password) {
        throw new Exception('Usunięcie konta z co najmniej jednym aktywnym zamówieniem jest niemożliwe.');
      }
      if (!password_verify($password_confirmation, $user_password)) {
        throw new Exception('Nieprawidłowe hasło. Spróbuj ponownie wprowadzając inne hasło.');
      }
