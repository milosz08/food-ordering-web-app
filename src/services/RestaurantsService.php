...
use App\Models\Cart\DishDetailsCartModel;
use App\Models\Rating\OpinionModel;
use App\Models\Restaurant\ListRestaurantModel;
use App\Models\Restaurant\RestaurantFilterModel;
use App\Models\Restaurant\RestaurantPersistFilterModel;
use App\Models\Restaurant\RestaurantWithDishesPageModel;
...
class RestaurantsService extends MvcService
{
  ...
  public function get_all_accepted_restaurants(): array
  {
    $pagination = array();
    $res_list = array();
    $pages_nav = array();
    ...
    try {
      ...
      if (empty($filter->open_selected)) {
        $filter->filter_query = str_replace($restaurant_open_query, '', $filter->filter_query);
      } else {
        $filter->filter_query .= $restaurant_open_query;
      }
      if (empty($filter->delivery_free_selected)) {
        $filter->filter_query = str_replace($delivery_free_query, '', $filter->filter_query);
      } else {
        $filter->filter_query .= $delivery_free_query;
      }
      if (empty($filter->has_discounts_selected)) {
        $filter->filter_query = str_replace($restaurant_has_discounts_query, '', $filter->filter_query);
      } else {
        $filter->filter_query .= $restaurant_has_discounts_query;
      }
      if (empty($filter->has_profile_selected)) {
        $filter->filter_query = str_replace($restaurant_has_images_query, '', $filter->filter_query);
      } else {
        $filter->filter_query .= $restaurant_has_images_query;
      }
      ...
      if (!empty($grade_stars)) {
        $filter->grade_stars['query'] = " AND
      ...
      for ($i = 0; $i < count($filter->grade_stars['data']); $i++) {
        ...
        $filter->grade_stars['data'][$i]['checked'] = $is_checked;
      }
      ...
      {
        $min_delivery_price = isset($_POST['restaurant-min-delivery-price'])
          ? filter_var($_POST['restaurant-min-delivery-price'], FILTER_SANITIZE_NUMBER_INT)
          : '-';
        $filter->min_delivery_price['price'] = $min_delivery_price;
      } else {
        $min_delivery_price = $filter->min_delivery_price['price'];
      }
      switch ($min_delivery_price) {
        case '0':
          $filter->min_delivery_price['query'] = " AND min_price IS NULL";
          break;
        case '35':
          $filter->min_delivery_price['query'] = " AND (CAST(min_price AS DECIMAL(10,2)) BETWEEN 0 AND 35)";
          break;
        case '50':
          $filter->min_delivery_price['query'] = " AND (CAST(min_price AS DECIMAL(10,2)) BETWEEN 0 AND 50)";
          break;
        case '51':
          $filter->min_delivery_price['query'] = " AND (CAST(min_price AS DECIMAL(10,2)) > 50)";
          break;
      }
      ...
      if (isset($_POST['filter-change'])) // sortowanie po dodatkowych parametrach
      {
        ...
        $filter->sort_parameters['sorted_by'] = $sorting_param;
      } else {
        $sorting_param = $filter->sort_parameters['sorted_by'];
      }
      ...
      if ($sorting_param != '-') {
        $filter->sorting_query = 'ORDER BY ' . $filter->sorting_query;
      }
      ...
      if (isset($_POST['filter-change'])) // ustawienie dodatkowego kierunku sortowania (rosnące/malejące)
      {
        ...
        $sorting_dir = $filter->sort_directions['dir'];
      }
        ...
        while ($row = $statement->fetchObject(OpinionModel::class)) {
          foreach ($res_list as $res) {
            if ($row->res_id == $res->id) {
              array_unshift($res->opinions, array('opinion' => $row));
            }
          }
        }
        $query = "
  ...
  public function get_restaurant_dishes_with_cart()
  {
    ...
    try {
      ...
      if (!$res_details) {
        $_banner_message = 'Wybrana restauracja nie istnieje, bądź nie jest otwarta.';
        SessionHelper::create_session_banner(SessionHelper::HOME_RESTAURANTS_LIST_PAGE_BANNER, $_banner_message, true);
        header('Location:' . __URL_INIT_DIR__ . 'restaurants', true, 301);
        die;
      }
      $this->dbh->prepare("SET lc_time_names = 'pl_PL'")->execute();
      ...
      $statement->execute(array($_GET['id']));
      while ($row = $statement->fetchObject(OpinionModel::class)) {
        array_unshift($res_details->opinions, array('opinion' => $row));
      }
      $query = "
      ...
      while ($row = $statement->fetchObject()) {
        ...
        // Wpisanie wszystkich szczegółów znalezionych dań pasujących do kategorii.
        while ($d_row = $d_statement->fetchObject()) {
          $dishes[] = $d_row;
        }
        // Uzupełnienie tablicy $dishTypes podaną kategorią, wraz z wszystkimi znalezionymi daniami.
        $dish_types[] = array('type' => $row, 'dishes' => $dishes);
        // Wyczyszczenie tablicy, aby przy następnym powtórzeniu nie wpisywały się poprzednie wartości
      ...
      if (isset($cookie)) {
        ...
        foreach ($cart_cookie['dishes'] as $dish) {
          ...
          if ($dish_details = $statement->fetchObject(DishDetailsCartModel::class)) {
            ...
            $shopping_cart[] = array('cart_dishes' => $dish_details, 'count_of_dish' => $dish['count']);
        ...
        if (!empty($cart_cookie['code'])) {
          ...
          if ($perc = $statement->fetch(PDO::FETCH_ASSOC)) {
            $summary_prices['percentage_discount'] = $perc['percentage_discount'];
          }
        }
