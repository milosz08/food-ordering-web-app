...
use App\Models\Discount\DiscountResDetailsModel;
use App\Models\Dish\DishModel;
use App\Models\Restaurant\AddEditRestaurantModel;
use App\Models\Restaurant\RestaurantDetailsModel;
...
class RestaurantsService extends MvcService
{
   ...
   */
  public function get_user_restaurants(): array
  {
    ...
    try {
      ...
      if (isset($_POST['restaurant-button'])) {
        ...
        foreach ($res_hours as $res_hour) {
          ValidationHelper::validate_hour($res_hour); // walidacja godzin
        }
        $all_hours_valid = true;
        foreach ($res_hours as $res_hour) {
          $all_hours_valid = $res_hour->all_hours_is_valid();
        }
        if ($res->all_is_valid() && $all_hours_valid) {
          ...
          $photos = ImagesHelper::upload_restaurant_images($res->profile_url, $rest_id);
          ...
          // aktualizowanie adresów profilu i banera w zmiennych po pobraniu zmienionych wartości w bazie danych
  ...
  private function check_if_phone_number_exist($phone_number)
  {
    ...
    if (!empty($statement->fetchColumn())) {
      throw new Exception('
        Podany numer telefonu jest już przypisany do restauracji innego właściciela. Taki sam numer telefonu możesz przypisać
        jedynie do restauracji stworzonych przez siebie i przypisanych do Twojego konta.
      ');
    }
    $statement->closeCursor();
  }
  ...
  public function edit_restaurant()
  {
    ...
    try {
      ...
      if (isset($_POST['restaurant-button'])) {
        ...
        if ($res->all_is_valid() && $all_hours_valid) {
          ...
          $photos = ImagesHelper::upload_restaurant_images($res->profile_url, $_GET['id'], $profile_photo, $banner_photo);
    ...
    return array(
      ...
      'res_id' => $_GET['id'],
      'res_hours' => $res_hours,
      'is_delivery_free' => $is_delivery_free ?? false,
      'is_no_min_price' => $is_min_price ?? false,
      'has_profile' => !empty($res->profile_url['value']),
      'has_banner' => !empty($res->banner_url['value']),
      'hide_profile_preview_class' => $res->profile_url['invalid'] ? 'display-none' : '',
      'hide_banner_preview_class' => $res->banner_url['invalid'] ? 'display-none' : '',
    );
  }
   ...
   */
  private function get_restaurant_weekdays_and_hours(): array
  {
    $ret_hours = array();
  ...
  public function delete_restaurant()
  {
    ...
    try {
      ...
      if ($statement->fetchColumn() != 0) {
        throw new Exception('
          Wybrana restauracja nie istnieje bądź posiada aktywne zamówienia. Z systemu możesz usunąć jednanie te restauracje,
          które nie mają w obecnej chwili aktywnych zamówień
        ');
      }
   ...
   */
  public function delete_restaurant_image($image_column_name, $deleted_type): string
  {
    $redirect_url = 'owner/restaurants';
    if (!isset($_GET['id'])) {
      return $redirect_url;
    ...
    try {
      ...
      $statement->execute(array($_GET['id'],));
      if ($statement->fetchColumn() == 0) {
        throw new Exception('Wybrana restauracja nie istnieje lub nie jest przypisana to Twojego konta.');
      }
      ...
      if (!$result) {
        throw new Exception('Wybrana restuaracja nie posiada typu zdjęcia: <strong>' . $image_column_name . '</strong>.');
      }
  ...
  public function get_restaurant_details(): array
  {
    ...
    try {
      ...
      foreach ($raw_res_hours as $raw_res_hour) {
        $res_hours[] = $raw_res_hour->format_to_details_view();
      }
      ...
      $statement->execute();
      while ($row = $statement->fetchObject(DiscountResDetailsModel::class)) {
        $restaurant_discounts[] = $row;
      }
      if ($this->dbh->inTransaction()) {
        $this->dbh->commit();
      }
