 ...
 *                                                             *
 * Nazwa pliku: DiscountsService.php                           *
...
use App\Models\DiscountRowModel;
use App\Models\ResDiscountsRowModel;
use App\Models\AddEditDiscountModel;
...
class DiscountsService extends MvcService
{
     ...
     */
    public function get_all_discounts_codes()
    {
        ...
        try
        {
            ...
            $statement->execute();
            while ($row = $statement->fetchObject(DiscountRowModel::class)) array_push($discount_codes, $row);
     ...
     */
    public function get_restaurants_with_discounts_codes()
    {
        ...
        try
        {
            ...
            $statement->bindValue('page', $page, PDO::PARAM_INT);
            $statement->execute();
            while ($row = $statement->fetchObject(ResDiscountsRowModel::class)) array_push($res_discount_codes, $row);
    ...
    {
        if (!isset($_GET['resid'])) header('Location:' . __URL_INIT_DIR__ . 'discounts/discounts-with-restaurants', true, 301);
        ...
        try
        {
            ...
            if (isset($_POST['add-edit-discount-code']))
            {
                ...
                if ($disc->all_is_valid())
                {
                    ...
                    ');
                    if (isset($_POST['disc-auto-generated'])) $disc->code['value'] = AuthHelper::generate_random_seq();
    ...
    {
        if (!isset($_GET['id']) || !isset($_GET['resid'])) header('Location:' . __URL_INIT_DIR__ .
            'discounts/discounts-with-restaurants', true, 301);
        $disc = new AddEditDiscountModel;
        try
        {
            $this->dbh->beginTransaction();
    ...
    {
        if (!isset($_GET['id']) || !isset($_GET['resid'])) header('Location:' . __URL_INIT_DIR__ .
            'owner/discounts/discounts-with-restaurants', true, 301);
        try
        {
            $this->dbh->beginTransaction();
            $discount_data = $this->check_if_discount_is_valid();
        ...
        $banner_type = isset($_GET['redir']) ? SessionHelper::DISCOUNTS_PAGE_BANNER : SessionHelper::RESTAURANT_DETAILS_PAGE_BANNER;
    ...
    public function increase_code_usages_count()
    {
        ...
        try
        {
            ...
            $query = "UPDATE discounts SET max_usages = max_usages + :incrusages WHERE id = :id";
            $statement = $this->dbh->prepare($query);
            $statement->bindValue('incrusages', $increase, PDO::PARAM_INT);
