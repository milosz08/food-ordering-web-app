...
use App\Models\Dish\DishOrderModel;
use App\Models\Rating\OwnerPendingToDeleteRatingModel;
...
class RatingsService extends MvcService
{
   ...
   */
  public function get_restaurants_ratings(): array
  {
    ...
    try {
      ...
      $query = "
        ...
        IF(g.id NOT IN((SELECT grade_id FROM notifs_grades_to_delete)), 'widoczna', 'do usunięcia') AS status,
        IF(g.id NOT IN((SELECT grade_id FROM notifs_grades_to_delete)), 'text-success', 'text-danger') AS status_bts_class
      ...
      if (!empty($restaurants_grades)) {
        $flatted_res = array_unique(array_map(function ($res_grades) {
          return $res_grades->res_id;
        }, $restaurants_grades));
        ...
        $statement->execute();
        $temp_dishes = array();
        while ($row = $statement->fetchObject(DishOrderModel::class)) {
          $temp_dishes[] = $row;
        }
        $statement->closeCursor();
        foreach ($temp_dishes as $dish) {
          foreach ($restaurants_grades as $res) {
            if ($dish->res_id = $res->res_id) {
              $res->order_dishes[] = array('dish' => $dish);
            }
          }
        }
      }
  ...
  public function send_request_to_delete_rating()
  {
    ...
    try {
      ...
      if (!$statement->fetchColumn()) {
        throw new Exception('Podany parametr ID typu zgłoszenia nie istnieje w systemie. Podaj inny parammetr.');
      }
      $query = "INSERT INTO notifs_grades_to_delete (description, type_id, grade_id) VALUES (NULLIF(?, ''),?,?)";
      $statement = $this->dbh->prepare($query);
   ...
   */
  public function get_pending_to_delete_ratings(): array
  {
    $filter_ratings = $_GET['restaurant'] ?? 'all';
    $pagination = array();
    $pages_nav = array();
    ...
    try {
      ...
      $statement->bindValue('page', $page, PDO::PARAM_INT);
      $statement->execute();
      while ($row = $statement->fetchObject(OwnerPendingToDeleteRatingModel::class)) {
  ...
  public function delete_pending()
  {
    ...
    try {
      $this->dbh->beginTransaction();
      $query = "
        SELECT gtd.grade_id FROM (((notifs_grades_to_delete AS gtd
        INNER JOIN restaurants_grades AS rg ON rg.id = gtd.grade_id)
        INNER JOIN orders AS o ON rg.order_id = o.id)
        INNER JOIN restaurants AS r ON o.restaurant_id = r.id)
      ...
      if (!$statement->fetchColumn()) {
        throw new Exception('
          Podane zgłoszenie z prośbą o usunięcie recenzji zostało już zaakceptowane, zostało odrzucone, bądź nie istnieje
          w systemie.
        ');
      }
