...
use App\Models\Auth\RegisterUserModel;
    ...
    if (isset($_POST['registration-button'])) {
      try {
        ...
        if (!empty($_POST['registration-local-number'])) {
          $user->locale_nr = ValidationHelper::validate_field_regex(
            'registration-local-number', Config::get('__REGEX_BUILDING_NO__')
          );
        } else {
          $user->locale_nr = array('value' => $_POST['registration-local-number'], 'invalid' => false, 'bts_class' => '');
        }
        ...
        if ($user->all_is_valid()) {
          ...
          if ($statement->fetchColumn() > 0) {
            throw new Exception('Podany użytkownik istnieje już w systemie. Podaj inne dane.');
          }
          ...
          $query = "
            INSERT INTO user_addresses (street, building_nr, locale_nr, post_code, city, is_prime, user_id)
            VALUES (?,?,NULLIF(?,''),?,?,1,?)
          ";
          $statement = $this->dbh->prepare($query);
          $statement->execute(array(
          ...
          SessionHelper::create_session_banner(SessionHelper::LOGIN_PAGE_BANNER, $_banner_message,
