...
class ForgotPasswordService extends MvcService
{
  ...
  /**
   * Metoda odpowiadająca za wysłanie żądania zmiany zapomnianego hasła. Metoda na podstawie podanego adresu email/loginu wysyłana na
  ...
  {
    $login_email = array('value' => '', 'invalid' => false, 'bts_class' => '');
  ...
  public function forgot_password_change()
  {
    ...
    $redirect_link = __URL_INIT_DIR__ . 'auth/forgot-password';
    $v_password = array('value' => '', 'invalid' => false, 'bts_class' => '');
    $v_password_rep = array('value' => '', 'invalid' => false, 'bts_class' => '');
    try {
      $this->dbh->beginTransaction();
      if (!isset($_GET['code'])) {
      ...
      $query = "
        SELECT user_id FROM ota_user_tokens
      ...
      if (empty($user_id)) {
        throw new Exception('
          ...
          <a class="alert-link" href="' . $redirect_link . '">kliknij tutaj</a>.
        ');
      }
      if (isset($_POST['form-send-change-pass'])) {
        ...
        if (!($v_password['invalid'] || $v_password_rep['invalid'])) {
          ...
          $redirect_link = __URL_INIT_DIR__ . 'auth/login';
