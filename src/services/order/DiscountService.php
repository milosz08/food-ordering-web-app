...
class DiscountService extends MvcService
{
    ...
    try {
      ...
      if (isset($_POST['discount-button']) && isset($cookie)) {
        ...
        if (!$percentage_discount) {
          throw new Exception('Podany kod rabatowy nie istnieje, nie jest przypisany do podanej restauracji bądź uległ już wygaśnięciu.');
        }
  ...
  {
    $cookie = $_COOKIE[CookieHelper::get_shopping_cart_name($_GET['resid'])] ?? null;
    if (!isset($_GET['resid']) || !isset($cookie)) {
      header('Location:' . __URL_INIT_DIR__ . 'restaurants', true, 301);
    }
    $temp_array = json_decode($cookie, true);
    $temp_array['code'] = '';
