 ...
 *                                                             *
 * Nazwa pliku: FeedbackService.php                            *
...
use App\Models\AddEditRestaurantGradeModel;
...
class FeedbackService extends MvcService
{
    ...
    {
        if (!isset($_GET['id'])) header('Location:' . __URL_INIT_DIR__ . 'user/orders', true, 301);
        ...
        try
        {
            ...
            if (isset($_POST['add-edit-mark']))
            {
                ...
                if (!isset($_POST['res-grade-stars'])) $grade->res_grade_stars['invl'] = true;
                if (!isset($_POST['delivery-grade-stars'])) $grade->delivery_grade_stars['invl'] = true;
    ...
    public function edit_mark_for_restaurant()
    {
        ...
        try
        {
            ...
            {
                if ($i + 1 <= $grade->restaurant_grade) $grade->res_grade_stars['data'][$i]['checked'] = 'checked';
                else $grade->res_grade_stars['data'][$i]['checked'] = '';
            }
            for ($i = 0; $i < count($grade->delivery_grade_stars['data']); $i++)
            {
                if ($i + 1 <= $grade->delivery_grade) $grade->delivery_grade_stars['data'][$i]['checked'] = 'checked';
                else $grade->delivery_grade_stars['data'][$i]['checked'] = '';
            }
            $grade->res_grade_stars['data'] = array_reverse($grade->res_grade_stars['data']);
            $grade->delivery_grade_stars['data'] = array_reverse($grade->delivery_grade_stars['data']);
