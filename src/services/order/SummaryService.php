...
class SummaryService extends MvcService
{
   ...
   * poprzez formularz składania zamówienia na stronie /order/summary. Dodatkowo wysyłanie wiadomości email do użytkownika i właściciela
   * restauracji.
   */
  public function place_new_order()
  {
    $cookie = $_COOKIE[CookieHelper::get_shopping_cart_name($_POST['resid'])] ?? null;
    if (!isset($_POST['resid']) || !isset($cookie)) {
      header('Location:' . __URL_INIT_DIR__ . 'restaurants', true, 301);
    }
    try {
      ...
      if (!empty($cart_cookie['code'])) // cena po rabacie jeżeli jest
      {
      ...
      // uzyskiwanie adresu
      $query = "SELECT count(*) FROM user_addresses WHERE user_id = ? AND id = ?";
      $statement = $this->dbh->prepare($query);
      ...
      $query = "
        INSERT INTO orders(user_id, discount_id, order_adress_id, delivery_type_id, restaurant_id, price, date_order, estimate_time)
      ...
      //pobieranie id generowanego zamówienia
      $query = "SELECT LAST_INSERT_ID()";
      $statement = $this->dbh->prepare($query);
      $statement->execute();
      $order_id = $statement->fetchColumn();

      //dodawanie zamówionych dań do orders_with_dishes
      foreach ($cart_cookie['dishes'] as $dish) {
  ...
  public function get_new_order_details()
  {
    ...
    try {
      ...
      $query = "SELECT count(*) FROM orders WHERE user_id = ? AND id = ? AND status_id = 1";
      $statement = $this->dbh->prepare($query);
      ...
      $query = "
        ...
        FROM orders o INNER JOIN user_addresses a ON o.order_adress_id = a.id WHERE o.id = ? AND o.user_id = ?
      ";
      $statement = $this->dbh->prepare($query);
