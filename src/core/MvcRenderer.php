...
class MvcRenderer
{
    ...
    {
        $template = self::$_mustache->loadTemplate('wrapper-view'); // załaduj główny szablon owijający pozostałe szablony
        $extended_data = array_merge($data, array(
            // wstaw funkcję wywołania zwrotnego szablonu renderowanego w szablonie owijającym
            'embed_rendering_content' => function () use ($pattern_name) { return '{{> ' . $pattern_name . ' }}'; },
            // suffix tytuły strony internetowej (patrz config.php)
            'suffix_page_title' => Config::get('__SUFFIX_PAGE_TITLE__'),
            // jeśli użytkonik ustawił niestandardowy tytuł, przypisz do embed_page_title, w przeciwnym razie przypisz wartość
    ...
    {
        if (!isset(self::$_singleton_instance)) self::$_singleton_instance = new MvcRenderer;
