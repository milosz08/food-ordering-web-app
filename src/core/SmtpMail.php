 ...
 *
 * Po więcej szczegółów na temat biblioteki, przejdź pod adres: https://github.com/PHPMailer/PHPMailer
 */
class SmtpMail
{
  ...
  private function __construct()
  {
    ...
    $this->_smtpClient->Port = Config::get('__SMTP_PORT__'); // standardowy port połączenia szyfrowanego dla połączenia protokołem SMTPS
    $this->_smtpClient->SMTPAuth = true; // uruchomienie możliwości uwierzytelniania serwera SMTP
    $this->_smtpClient->Username = Config::get('__SMTP_USERNAME__'); // nazwa użytkownika serwera SMTP
    $this->_smtpClient->Password = Config::get('__SMTP_PASSWORD__'); // hasło do konta email z serwera SMTP
    $this->_smtpClient->SMTPSecure = Config::get('__SMTP_ENCRYPTION_TYPE__'); // ustawienie szyfrowania
  }

  /**
   * Metoda statyczna umożliwiająca stworzenie klasy SmtpMail. Uruchomić można ją tylko raz (tylko raz dojdzie do stworzenia
   * obiektu). Jeśli obiekt będzie już istniał, pobierze referencję do niego z pamięci.
   */
  public static function get_instance(): SmtpMail
  {
    if (self::$_instance == null) {
      self::$_instance = new SmtpMail;
    }
   ...
   *      $this->smtp_client->send_message('example@email.com', 'To jest testowy temat', 'test-email', $variables);
   *
   ...
   */
  public function send_message($sendTo, $subject, $body_template_path, $template_vars = array(), $attachments_path = array()): array
  {
    $send_status_error = false;
    try {
      $this->_smtpClient->setFrom(Config::get('__SMTP_SENDER__'), 'Dobre.pl'); // ustaw adres serwera SMTP
      ...
      foreach ($attachments_path as $attachment_path) {
        $this->_smtpClient->addAttachment($attachment_path);
      }
      $this->_smtpClient->CharSet = 'UTF-8'; // wymuszenie kodowania znaków na UTF-8
      $this->_smtpClient->isHTML(); // ustawienie wysyłanego kontentu jako text/html
      ...
      if (!$body_content) {
        throw new Exception(); // jeśli nie znajdzie pliku, wyrzuć wyjątek
      }
      ...
      // jeśli nie zdoła wysłać wiadomości email na jeden z podanych adresów, przechwyć wyjątek
    } catch (Exception $e) {
